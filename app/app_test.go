...
func TestTx(t *testing.T) {
	...
	reset := func() {
		...
		reswrsp := app.Commit()
		assert.True(reswrsp.IsOK(), reswrsp)
	}
	reset()
	...
	exec := func(checkTx bool) (res wrsp.Result, foo, fooExp, bar, barExp types.Coins) {
	...
	resQueryPreCommit := app.Query(wrsp.RequestQuery{
		Path: "/account",
	...
	resQueryPostCommit := app.Query(wrsp.RequestQuery{
		Path: "/account",
		Data: accsFoo[0].Account.PubKey.Address(),
	})
