...
func (tv *testValues) appInit() {
	...
	reswrsp := tv.app.Commit()
	require.True(tv.t, reswrsp.IsOK(), reswrsp)
}
...
func (tv testValues) exec(tx *types.SendTx, checkTx bool) (res wrsp.Result, foo, fooExp, bar, barExp types.Coins) {
...
func TestQuery(t *testing.T) {
	...
	resQueryPreCommit := tv.app.Query(wrsp.RequestQuery{
		Path: "/account",
	...
	resQueryPostCommit := tv.app.Query(wrsp.RequestQuery{
		Path: "/account",
		Data: tv.accsFoo[0].Account.PubKey.Address(),
	})
