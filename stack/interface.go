...
type InitValidaterMiddle interface {
	InitValidate(l log.Logger, store state.SimpleDB, vals []*wrsp.Validator, next basecoin.InitValidater)
}

type InitValidaterMiddleFunc func(log.Logger, state.SimpleDB,
	[]*wrsp.Validator, basecoin.InitValidater)

func (c InitValidaterMiddleFunc) InitValidate(l log.Logger, store state.SimpleDB,
	vals []*wrsp.Validator, next basecoin.InitValidater) {
	c(l, store, vals, next)
}
