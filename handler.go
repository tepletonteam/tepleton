...
type Handler interface {
	...
	// InitChain(store state.KVStore, vals []*wrsp.Validator)
	// BeginBlock(store state.KVStore, hash []byte, header *wrsp.Header)
	// EndBlock(store state.KVStore, height uint64) wrsp.ResponseEndBlock
}
