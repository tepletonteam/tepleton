...
type Ticker interface {
	Tick(Context, SimpleDB) ([]*wrsp.Validator, error)
}

// TickerFunc allows a function to implement the interface
type TickerFunc func(Context, SimpleDB) ([]*wrsp.Validator, error)

func (t TickerFunc) Tick(ctx Context, store SimpleDB) ([]*wrsp.Validator, error) {
	return t(ctx, store)
}
