...
// get cmd to initialize all files for tepleton testnet and application
...
Example:
	...
	tond testnet --v 4 --output-dir ./output --starting-ip-address 192.168.10.2
	`,
...
func testnetWithConfig(config *cfg.Config, cdc *wire.Codec, appInit AppInit) error {
	...
	for i := 0; i < viper.GetInt(nValidators); i++ {
		...
		nodeDir := filepath.Join(outDir, nodeDirName, "tond")
		clientDir := filepath.Join(outDir, nodeDirName, "toncli")
