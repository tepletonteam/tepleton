...
docker build -t tepleton/${REPO_NAME}-builder scripts/${REPO_NAME}-builder/
docker run --rm -e "BUILD_TAGS=$BUILD_TAGS" -v "$(pwd)":/go/src/github.com/tepleton/${REPO_NAME} tepleton/${REPO_NAME}-builder ./scripts/dist_build.sh
