...
func (et *execTest) exec(tx *types.SendTx, checkTx bool) (res wrsp.Result, inGot, inExp, outGot, outExp types.Coins) {
...
func TestValidateInputAdvanced(t *testing.T) {
	...
	et.signTx(tx, et.accIn, et.accOut)
	res = validateInputAdvanced(&et.accIn.Account, signBytes, tx.Inputs[0])
	assert.Equal(wrsp.CodeType_BaseInvalidSequence, res.Code, "validateInputAdvanced: expected error on tx input with bad sequence")
	...
	et.signTx(tx, et.accIn, et.accOut)
	res = validateInputAdvanced(&et.accIn.Account, signBytes, tx.Inputs[0])
	assert.Equal(wrsp.CodeType_BaseInsufficientFunds, res.Code,
