...
func ExecTx(state *State, tx types.Tx, isCheckTx bool, evc events.Fireable) wrsp.Result {
	...
	case *types.CallTx:
		...
		if strings.HasPrefix(string(tx.Address), "gov/") {
			...
			return wrsp.ErrBaseInvalidAddress.AppendLog(Fmt("Unrecognized address %X", tx.Address))
		}
...
func getInputs(state types.AccountGetter, ins []types.TxInput) (map[string]*types.Account, wrsp.Result) {
	...
	for _, in := range ins {
		...
		if acc == nil {
			return nil, wrsp.ErrBaseInvalidAddress
		}
...
func checkInputPubKey(address []byte, acc *types.Account, in types.TxInput) wrsp.Result {
	if acc.PubKey == nil {
		if in.PubKey == nil {
			return wrsp.ErrBaseUnknownPubKey
		}
		if !bytes.Equal(in.PubKey.Address(), address) {
			return wrsp.ErrBaseInvalidPubKey
		}
...
func validateInputs(state *State, accounts map[string]*types.Account, signBytes []byte, ins []types.TxInput) (total int64, res wrsp.Result) {
...
func validateInput(state *State, acc *types.Account, signBytes []byte, in types.TxInput) (res wrsp.Result) {
	// Check TxInput basic
	...
	if !acc.PubKey.VerifyBytes(signBytes, in.Signature) {
		return wrsp.ErrBaseInvalidSignature
	}
	return wrsp.OK
}
