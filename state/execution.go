...
func ExecTx(s *State, pgz *types.Plugins, tx types.Tx, isCheckTx bool, evc events.Fireable) wrsp.Result {
...
func checkInputPubKey(address []byte, acc *types.Account, in types.TxInput) wrsp.Result {
	if acc.PubKey == nil {
		if in.PubKey == nil {
			return wrsp.ErrBaseUnknownPubKey.AppendLog("PubKey not present in either acc or input")
		}
		if !bytes.Equal(in.PubKey.Address(), address) {
			return wrsp.ErrBaseInvalidPubKey.AppendLog("Input PubKey address does not match address")
		}
...
func validateInputs(accounts map[string]*types.Account, signBytes []byte, ins []types.TxInput) (total types.Coins, res wrsp.Result) {
...
func validateInput(acc *types.Account, signBytes []byte, in types.TxInput) (res wrsp.Result) {
	// Check TxInput basic
...
func validateOutputs(outs []types.TxOutput) (total types.Coins, res wrsp.Result) {
	for _, out := range outs {
