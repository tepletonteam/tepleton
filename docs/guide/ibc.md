...
In previous examples, we started basecoin in-process with tepleton.  Here, we
will run them in different processes, using the `--without-tepleton` flag, as
...
``` TMROOT=$BCHOME2 tepleton node --log_level=info --node_laddr
tcp://localhost:36656 --rpc_laddr tcp://localhost:36657 --proxy_app
tcp://localhost:36658 &> chain2_tepleton.log & BCHOME=$BCHOME2 basecoin start
--address tcp://localhost:36658 --without-tepleton &> chain2_basecoin.log &
```
