...
but we'll explain how IBC works. You may also want to see [our repository of example plugins](https://github.com/tepleton/basecoin-examples).
...
We can query the Merkle tree using the WRSP Query method.
...
In previous examples, we started basecoin in-process with tepleton.
Here, we will run them in different processes, using the `--without-tepleton` flag,
as described in the [guide to the basecoin tool](basecoin-tool.md).
We can start the two chains as follows:

```
TMROOT=$BCHOME1 tepleton node --log_level=info &> chain1_tepleton.log &
BCHOME=$BCHOME1 basecoin start --without-tepleton &> chain1_basecoin.log &
```

and

```
TMROOT=$BCHOME2 tepleton node --log_level=info --node_laddr tcp://localhost:36656 --rpc_laddr tcp://localhost:36657 --proxy_app tcp://localhost:36658 &> chain2_tepleton.log &
BCHOME=$BCHOME2 basecoin start --address tcp://localhost:36658 --without-tepleton &> chain2_basecoin.log &
```
