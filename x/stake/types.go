...
func (vs Validators) validatorsUpdated(vs2 Validators) (updated []*wrsp.Validator) {
	...
	max := len(vs) + len(vs2)
	updated = make([]*wrsp.Validator, max)
	...
	for i < len(vs) && j < len(vs2) {
		...
		if !vs[i].PubKey.Equals(vs2[j].PubKey) {
			...
			if bytes.Compare(vs[i].PubKey.Bytes(), vs2[j].PubKey.Bytes()) == 1 {
				updated[n] = vs2[j].WRSPValidator()
				n++
				j++
				continue
			} // else, the old validator has been removed
			updated[n] = &wrsp.Validator{vs[i].PubKey.Bytes(), 0}
			n++
			i++
			continue
		}
...
func UpdateValidatorSet(store types.KVStore, gs *GlobalState, params Params) (change []*wrsp.Validator, err error) {
